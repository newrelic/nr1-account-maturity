export default {
  ...
  // nrqlQueries: (entity) => ({
  // pixie: `FROM Span SELECT count(*) where instrumentation.provider = 'pixie' AND service.name is not null AND k8s.cluster.name = '${entity.name}' since 30 hours ago LIMIT MAX`,
  // prometheusLabels: `SELECT uniqueCount(clusterName) as 'clusterName' FROM Metric WHERE integrationName ='nri-prometheus' OR instrumentation.provider = 'prometheus' AND clusterName = '${entity.name}' since 30 hours ago LIMIT MAX`,
  // logEvents: `FROM Log SELECT count(*) where plugin.source = 'kubernetes' AND cluster_name = '${entity.name}' since 30 hours ago LIMIT MAX`,
  // apmAgents: `FROM Transaction SELECT uniqueCount(appName) as 'appName' WHERE clusterName IS NOT NULL AND clusterName = '${entity.name}' since 30 hours ago LIMIT MAX`,
  // infraAgents: `FROM K8sClusterSample SELECT uniqueCount(clusterName) as 'clusterName' WHERE agentName = 'Infrastructure' AND clusterName = '${entity.name}' since 30 hours ago LIMIT MAX`,
  // }),
